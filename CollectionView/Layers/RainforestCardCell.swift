...
class RainforestCardCell: UICollectionViewCell {
    ...
    override func sizeThatFits(_ size: CGSize) -> CGSize {
        if let featureImageSize = featureImageSizeOptional {
            return FrameCalculator.sizeThatFits(size: size, withImageSize: featureImageSize)
        } else {
            return CGSize.zero
        }
    }
    ...
    override func layoutSubviews() {
        ...
        let featureImageSize = featureImageSizeOptional ?? CGSize.zero
    ...
    func configureCellDisplayWithCardInfo(cardInfo: RainforestCardInfo) {
        ...
        backgroundImageView.image = image.applyBlur(withRadius: 30, tintColor: UIColor(white: 0.5, alpha: 0.3),
        ...
        titleLabel.attributedText = NSAttributedString.attributedStringForTitleText(text: cardInfo.name)
        
        descriptionTextView.backgroundColor = UIColor.clear
        descriptionTextView.isEditable = false
        descriptionTextView.isScrollEnabled = false
        descriptionTextView.attributedText = NSAttributedString.attributedStringForDescriptionText(text: cardInfo.description)
